...
class ChainInfoView extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.pastVotes !== this.state.pastVotes) {
      ...
      const topAnimal = [...animalMap.entries()].reduce((a, e ) => e[1] > a[1] ? e : a);
      const foundAnimal = animalData.find((animal) => animal.animalName === topAnimal[0]);
  ...
  render() {
    return (
      <Container id="info-box" maxWidth = {false} style = {{textAlign: 'center'}} >
        <h1>Currently on {this.props.networkName} network.</h1>
        <h2>The top animal on this chain is a {this.state.topAnimalName} with {this.state.topAnimalVotes} votes!</h2>
        <Image src={this.state.topAnimalPic} style={{width: '20rem', borderStyle: 'solid',
